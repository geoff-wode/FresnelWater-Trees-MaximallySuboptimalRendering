...
namespace Meadow.Geology
{
    ...
    public class Terrain : SceneNode, ITerrainInfo
    {
        private struct VertexPositionNormalMultiTexture
        {
            ...
            {
                new VertexElement( 0, 0, VertexElementFormat.Vector3, VertexElementUsage.Position, 0 ),
                new VertexElement( 0, sizeof(float) * 3, VertexElementFormat.Vector3, VertexElementUsage.Normal, 0 ),
                new VertexElement( 0, sizeof(float) * 6, VertexElementFormat.Vector2, VertexElementUsage.TextureCoordinate, 0 ),
                new VertexElement( 0, sizeof(float) * 8, VertexElementFormat.Vector4, VertexElementUsage.TextureCoordinate, 1 )
            };
        }
        ...
        private void LoadHeightMap()
        {
            ...
            // Load the heighs as values in [0, 1] range...
            for (int z = 0; z < h; z++)
            {
                for (int x = 0; x < w; x++)
                {
                    float height = colours[x + (z * w)].R / 256f;
            ...
            // The "range" sets the maximum height in the world that is represented by '1' in the height data...
            float range = 30;
            ...
            for (int z = 0; z < h; z++)
            {
                for (int x = 0; x < w; x++)
                {
                    ...
                    heightMap[x, z] = (heightMap[x, z] - minHeight) * range;
                }
            }
        }
        ...
        private float GetExactHeight(float x, float z)
        {
            ...
            int lowerX = (int)x % grid.Width;
            int upperX = (lowerX + 1) % grid.Width;
            float relativeX = (x - lowerX) / (float)(upperX - lowerX);

            int lowerZ = (int)z % grid.Height;
            int upperZ = (lowerZ + 1) % grid.Height;
